...
MNT=/tmp/mnt
LABELS_FILE=$MNT/channel-labels.xml
...
read_labels_xml() {
    ...
    awk '
        match($0, /<channel[[:space:]]+index="[0-9]+"[[:space:]]+label="[^"]*"[[:space:]]*\/>/) {
            line = substr($0, RSTART, RLENGTH)
            idx = line
            lbl = line
            sub(/^.*index="/, "", idx); sub(/".*$/, "", idx)
            sub(/^.*label="/, "", lbl); sub(/"[[:space:]]*\/?>.*$/, "", lbl)
            printf "%s\t%s\n", idx, lbl
        }' "$LABELS_FILE" | while IFS="$(printf '\t')" read -r idx lbl; do
            printf '%s\t%s\n' "$idx" "$(xml_unescape "$lbl")"
        done
}
...
write_labels_xml() {
    ...
    tmpfile=$(mktemp)
    {
        ...
        printf '<channelLabels channels="%s">\n' "$channels"
        while IFS= read -r line; do
            ...
            esc=$(xml_escape "$raw")
            printf '  <channel index="%s" label="%s"/>\n' "$idx" "$esc"
        done
        printf '%s\n' '</channelLabels>'
    } > "$tmpfile"
...
labels_info() {
    ...
    read_labels_xml | while IFS="$(printf '\t')" read -r idx lbl; do
        printf 'LABEL[%s]=%s\n' "$idx" "$lbl"
    done
}
...
write_labels_from_stdin() {
    ...
    write_labels_xml "$ch"
    echo "Labels saved to $LABELS_FILE"
}
...
reply_labels_get() {
    ...
    json_add_string file "${file:-$LABELS_FILE}"
    json_add_string message "$output"
    json_add_object labels
    printf '%s\n' "$output" | awk '/^LABEL\[[0-9]+\]=/ { print }' | while IFS= read -r line; do
...
return view.extend({
    render: function() {
        ...
        function setButtonsDisabled(disabled) {
            ...
            if (saveButton)
                saveButton.disabled = disabled || currentRunning || currentChannels < 1;
        ...
        function buildLabelsTable(channels, labels) {
            ...
            labelsInfo.textContent = _('Detected channels: ') + String(channels) + _(' — labels are saved as XML in the recordings directory.');
            setButtonsDisabled(false);
        }
        ...
        function refreshLabels() {
            return callLabelsGet().then(function(res) {
                ...
                buildLabelsTable(res.channels || 0, res.labels || {});
            }).catch(function(err) {
        ...
        return E('div', { 'class': 'cbi-map' }, [
            ...
            E('div', { 'class': 'cbi-map-descr' },
                _('Control the autorecorder daemon and store channel labels in an XML file inside the recordings directory. Channel-label editing is enabled only when the recording service is stopped.')),
